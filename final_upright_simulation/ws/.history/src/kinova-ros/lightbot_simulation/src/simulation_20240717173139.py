...
def count_valid_poses(camera_x, camera_y, camera_z, goal_poses, move_group, tf_buffer):
    ...
    update_camera_position(camera_x, camera_y, camera_z, method='tf')  # or 'tf'
    ...
    for r, theta, phi in goal_poses:
        ...
        try:
            ...
            # Check if the pose is reachable using IK service
            ik_request = GetPositionIKRequest()
            ik_request.ik_request.group_name = move_group.get_name()
            ik_request.ik_request.robot_state = move_group.get_current_state()
            ik_request.ik_request.pose_stamped = pose_transformed
            ...
            if ik_response.error_code.val == ik_response.error_code.SUCCESS:
                # IK solution found, now check for collisions
                ...
                if isinstance(plan, tuple):
                    ...
                    success = plan.joint_trajectory.points  # If there are points, consider it successful
                    trajectory = plan
...
if __name__ == '__main__':
    try:
        ...
        y_range = (0.15, 0.75)  # Y range
        z_range = (0.1, 0.5)   # Z range
        y_step = 0.05
        z_step = 0.05
